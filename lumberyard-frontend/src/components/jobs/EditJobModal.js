...
const EditJobModal = ({ job, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (job) {
      setFormData({
        jobId: job.jobId || '',
        jobName: job.jobName || '',
        customJobName: job.jobName && !JOB_NAMES.includes(job.jobName) ? job.jobName : '',
        date: job.date || new Date().toISOString().split('T')[0],
        status: job.status || 'PENDING'
      });
      
      // Set position quantities
      if (job.positionRequirements && Object.keys(job.positionRequirements).length > 0) {
        setPositionQuantities(job.positionRequirements);
        setShowCustomInput(false);
      } else {
        // Legacy mode - convert to positions
        const legacyPositions = {};
        if (job.requiredEmployees > 0) {
          legacyPositions['Employee'] = job.requiredEmployees;
        }
        if (job.requiredSupervisors > 0) {
          legacyPositions['Supervisor'] = job.requiredSupervisors;
        }
        setPositionQuantities(legacyPositions);
      }
      
      // Check if it's a custom job name
      if (job.jobName && !JOB_NAMES.includes(job.jobName)) {
        setShowCustomInput(true);
        setFormData(prev => ({ ...prev, customJobName: job.jobName }));
      }
    }
  }, [job]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Updating job:', job.id, payload);
      const response = await API.put(`/jobs/${job.id}`, payload);
      ...
      <div className="create-job-modal" onClick={e => e.stopPropagation()}>
        ...
        <form onSubmit={handleSubmit} className="modal-form">
          ...
          <div className="form-group">
            ...
            />
            <small className="form-hint">Job ID cannot be changed</small>
          </div>
