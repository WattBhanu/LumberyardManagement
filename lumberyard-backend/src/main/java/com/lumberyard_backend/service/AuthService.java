...
public class AuthService {
    ...
    private void authenticateUser(String username, String password) throws Exception {
        try {
            // Since we're allowing login by email, phone, or username, 
            // we'll use the email as the principal for authentication
            User user = userRepository.findByEmail(username)
                    .orElse(userRepository.findByPhone(username).orElse(null));
            ...
            authenticationManager.authenticate(
                new UsernamePasswordAuthenticationToken(
                    user.getEmail(), // Use email as principal for Spring Security
                    password
                )
            );
